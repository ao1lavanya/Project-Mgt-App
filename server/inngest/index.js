...
const syncWorkSpaceMemberCreation = inngest.createFunction(
    ...
    {event: 'clerk/organizationInvitation.member.accepted'},
    async({event})=>{
        const {data} = event;
        await prisma.workspaceMember.create({
            data:{
...
const sendTaskAssignmentEmail = inngest.createFunction(
    ...
    async({event,step})=>{
       ...
       if(new Date(task.due_date).toLocaleDateString() !== new Date().toLocaleDateString()){
        // schedule reminder email 1 day before due date
          ...
          if(task.status !== "DONE"){
            await step.run('send-task-reminder-mail', async()=>{
              await sendEmail({
                ...
                body: `<div style="max-width: 600px;">
                      <h2>Hi ${task.assignee.name}, 👋</h2>
                     <p style="font-size: 16px;">This is a friendly reminder that the following task is due soon:</p>
                     <p style="font-size: 18px; font-weight: bold; color: #007bff;
                     margin: 8px 0;">${task.title}</p>
